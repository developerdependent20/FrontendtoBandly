        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 99999, background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }} onClick={resetCreateForm}>
          <div className="glass-panel" style={{ width: '100%', maxWidth: '440px', padding: '2rem' }} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.2rem' }}>
              <h3 style={{ margin: 0, fontSize: '1.2rem', fontWeight: '800' }}>Nueva Encuesta</h3>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '1rem' }}>
              {options.map((opt, idx) => (
                <div key={idx} style={{ display: 'flex', gap: '8px' }}>
                  ...
                  />
                  {options.length > 2 && (
                    <button onClick={() => handleRemoveOptionField(idx)} style={{ background: 'rgba(239,68,68,0.1)', border: 'none', borderRadius: '8px', color: '#ef4444', cursor: 'pointer', padding: '0 12px' }}>
                      <X size={16} />
                    </button>
                  )}
                </div>
            ...
            <button onClick={handleAddOptionField} style={{ background: 'rgba(255,255,255,0.05)', border: '1px dashed rgba(255,255,255,0.2)', borderRadius: '10px', color: 'var(--text-muted)', cursor: 'pointer', padding: '0.6rem', width: '100%', fontSize: '0.8rem', fontWeight: '700', marginBottom: '1.5rem' }}>
              + Agregar opción
            </button>
            ...
            return (
              <div key={poll.id} className="glass-panel" style={{ padding: '1.5rem', opacity: poll.is_closed ? 0.7 : 1 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '10px', marginBottom: '0.3rem' }}>
                  <h3 style={{ margin: 0, fontSize: '1.05rem', fontWeight: '800', color: 'white' }}>{poll.question}</h3>
                  {canManage && (
                    <div style={{ display: 'flex', gap: '6px', flexShrink: 0 }}>
                      <button onClick={() => handleToggleClose(poll)} title={poll.is_closed ? 'Reabrir' : 'Cerrar encuesta'} style={{ background: 'rgba(255,255,255,0.05)', border: 'none', borderRadius: '8px', color: 'var(--text-muted)', cursor: 'pointer', padding: '6px' }}>
                        <Lock size={14} />
                      </button>
                      <button onClick={() => handleDeletePoll(poll)} title="Borrar" style={{ background: 'rgba(239,68,68,0.08)', border: 'none', borderRadius: '8px', color: '#ef4444', cursor: 'pointer', padding: '6px' }}>
                        <Trash2 size={14} />
                      </button>
                    </div>
                  )}
                  ...
                  {options.map(opt => {
                    ...
                    return (
                      <button
                        ...
                        style={{
                          position: 'relative', textAlign: 'left', padding: '0.7rem 1rem', borderRadius: '10px', overflow: 'hidden',
                      ...
                      >
                        <div style={{ position: 'absolute', top: 0, left: 0, bottom: 0, width: `${pct}%`, background: isMine ? 'rgba(139,92,246,0.25)' : 'rgba(255,255,255,0.06)', transition: 'width 0.3s' }} />
                        <div style={{ position: 'relative', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                          ...
                          <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', fontWeight: '700' }}>{pct}% ({count})</span>
                        </div>
                      </button>
                    );
                  })}
